...
from keyboards import share_phone_btn, storage_inline_btn, get_languages_btn
from messages import messages
from states import StudentRegistrationForm
from task import Chat, Task, Storage
from utils import get_fullname, write_to_csv, get_language_code_by_chat_id
...
def age_get(message):
    bot.send_message(message.chat.id, "Tilni kiriting:", reply_markup=get_languages_btn("language"))
    bot.set_state(message.from_user.id, StudentRegistrationForm.language, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data["age"] = message.text


@bot.callback_query_handler(lambda call: call.data.startswith("language"), state=StudentRegistrationForm.language)
...
@bot.callback_query_handler(lambda c: c.data.startswith("course"), state=StudentRegistrationForm.course)
def course_get(call):
    message = call.message
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        bot.send_message(message.chat.id, msg, parse_mode="html", reply_markup=storage_inline_btn("storage"))
    bot.delete_state(message.from_user.id, message.chat.id)


@bot.callback_query_handler(func=lambda call: call.data.startswith("storage"))
def callback(call):
    message = call.message
    ...
    if text == "OK":
        ...
        write_to_csv(
            ...
            save_info.get_storage_info_csv()
        )
    elif text == "NO":
